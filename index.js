...
function dizi25Cesitmi(orijinalTat){
 if (orijinalTat.length===25) {
  return true;
 }
 else{
  return false;
...
function cesitEkle(ya,yeni){

  // let yeniDizi =[...orijinalTatlar];
   ya.unshift(yeni);
  return ya;
...
function indekstekiCesitiGetir(orijinalTatlar,sayı){
  let yeniDizi =[...orijinalTatlar];
  console.log(yeniDizi);
  return yeniDizi[sayı];
...
function ismeGoreCesitCikar(gelendizi,yeniTatlar){

  let indeksteki= gelendizi.indexOf(yeniTatlar);

  gelendizi.splice(indeksteki,1);
  return gelendizi ;
...
function ismeGoreFiltrele(dizi,aroma){
  let filtre=[];
 for (let i = 0; i < dizi.length; i++) {
   if (dizi[i].includes(aroma)) {
    filtre.push(dizi[i]);
   } 
 }
 return filtre;
...
function rastgeleTatlar(orijinal,yeni,mevsimlik,bölgesel){
 let toplamDizi= orijinal.concat(yeni,mevsimlik,bölgesel);
  let rastgele=  Math.floor(Math.random()*25);
  let yeniDizi= toplamDizi[rastgele];
     return yeniDizi;
}
